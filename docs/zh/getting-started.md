...
可以检查三样东西，它们回答的是三个不同的问题。

**校验和 —— 文件是否完整到达。** 压缩包旁边的 `.sha256` 文件，用上面的
`certutil` 比对。PowerShell 安装脚本自己就会做这件事，对不上就拒绝安装。它说不了
的是压缩包的来路：校验和跟下载放在同一个页面上，能换掉其中一个的人，另一个也能
换掉。

**证明 —— 它从哪里来。** 从 1.9.0 起，每个版本在构建的那一刻通过 Sigstore 签名，
在 GitHub 的运行器上，用的是短命的身份，而不是谁保管的密钥：

```powershell
gh attestation verify portable.zip -R dskripchenko/portable
```

回答里有仓库、工作流，以及压缩包是从哪个提交构建出来的。它需要
[GitHub CLI](https://cli.github.com) —— 那是另一次下载，在受管控的机器上本身也
可能被封；那就还剩校验和这条路。

**VirusTotal 报告 —— 扫描引擎怎么看。** 每个版本的说明里都有链接，约七十个引擎。

预期是有几处报毒，而不是干干净净。这个工具下载可执行文件、解包、维持一池脱离控制台
的进程、占用 80 端口，而且它把一个解释器装在 zip 里发出去 —— 那正是加壳程序的形状。
启发式规则就是为了盯住这些而写的。报告摆在那里是供人读的，不是用来证明什么是干净的。

**压缩包没有代码签名证书。** 这类证书每年要花钱，而这个项目不收钱，所以 SmartScreen
会对下载发出警告。这一点在这里改不了，而建议你关掉防护，是比那条警告更糟的交换。
...
它会把这个文件夹之外的一切收回来：无论你把数据目录放在哪儿、你是否加过 PATH 记
录、是否信任过证书，以及升级留下的任何副本。它会先把找到的东西连同体积一起列出
来，然后再问你。

之后删掉你解压出来的文件夹，这一切就像从未存在过。没有注册表项，没有服务，什么都
不会留下。
