...
`trust` 把本地颁发机构的根放进**你的**证书存储 —— 唯一不需要管理员权限的那个 ——
并写出 `conf/ca-bundle.pem`：这台机器自己信任的根，加上那个根。每个已安装的 PHP 都
会被指向它。

PHP、curl 和 Node 读的是各自的清单，而不是系统存储。所以站点在 Chrome 里是绿色的，
而同一个站点代码里的 `file_get_contents('https://api.localhost')` 却栽在证书上。
`portable run` 和 `portable env` 会把同一个文件交给 curl 和 Node。

机器自己的根是特意放进去的：只装本地颁发机构的包会让 PHP 信任 `api.localhost`，同时
拒绝所有公网证书。如果这些根收集不到，就什么都不写，`trust` 会直说。

不需要为此重新跑一次 `trust`：只要颁发机构存在，而文件缺失或比它旧，监督进程就会在
启动时写出这个文件。

如果 443 和 8443 都被占用，就根本不会启动 HTTPS 监听 —— HTTP 不受影响。`status` 会
说明这一点，并指出是谁占着它们。
