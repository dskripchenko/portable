...
它不该死。从控制台、进程组和作业对象上脱离都是显式处理过的 —— 但这是尚未在真实
Windows 上确认的两件事之一，而 IDE 尤其喜欢把子进程塞进一个“关闭即杀”的作业对象里。

如果发生了，请报告你是从哪个终端或哪个 IDE 启动的。这个细节就是全部诊断。
